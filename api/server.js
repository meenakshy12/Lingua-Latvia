...
[{ "word": "hello", "clue": "sveiki" }]. Ensure the words are unique and do not repeat any previously provided words.`,
    ...
    {
      role: "user",
      content: `Provide five unique Latvian words with their English meanings.Focus on the difficulty level: ${difficulty}`,
    },
  ];
...
app.post("/game02", async (req, res) => {
  try {
    ...
    const MAX_ATTEMPTS = 5; // Increase attempts to 5 to try to get at least 10 valid questions
    let allQuestions = [];
    let attempts = 0;
...
${parseData}`,
        ...
        {
          role: "user",
          content: `Provide at least 40 unique Latvian vocabulary questions in the specified format. Focus on the difficulty level: ${difficulty}. For higher difficulty levels, generate more complex and vocabulary questions appropriate for the user's proficiency.
...
- Only return an array of 40 objects in the required JSON format.
...
`,
      ...
      // Log raw AI response content for debugging
      console.log("Raw AI response content:", content);

      // Validate and parse the JSON response
      let questions;
      try {
        // Strictly extract JSON array from the response
        const cleanedContent = content.replace(/```json|```/g, "").trim();
        // Use a more robust regex to extract JSON array
        const arrayMatch = cleanedContent.match(/\[\s*(?:\{[^]*?\}\s*,?\s*)+\]/);
        if (!arrayMatch) {
          console.warn("No JSON array found in AI response, attempting fallback parse.");
          questions = JSON.parse(cleanedContent);
        } else {
          questions = JSON.parse(arrayMatch[0]);
        }
        if (!Array.isArray(questions) || questions.some(q => !q.sentence || !q.options || !q.correctAnswer)) {
          throw new Error("Invalid response format");
        }
        // Additional check to avoid error when q.translation is undefined
        questions = questions.filter(q => q.translation !== undefined);
      } catch (parseError) {
        console.error("Error parsing JSON response:", parseError);
        console.error("Raw AI response content on parse error:", content);
      ...
      questions = questions.filter(q => {
        // Add logging for debugging filter rejections
        if (!isLikelyNominative(q.correctAnswer)) {
          console.log(`Filtered out question due to correctAnswer not nominative: ${q.correctAnswer}`);
          return false;
        }
        if (!q.options.every(isLikelyNominative)) {
          console.log(`Filtered out question due to options not all nominative: ${q.options.join(", ")}`);
          return false;
        }
        if (!q.options.every(isValidLatvianWord)) {
          console.log(`Filtered out question due to invalid Latvian words in options: ${q.options.join(", ")}`);
          return false;
        }
        // Relax adjective ending matching filter: comment out for now
        // if (!adjectiveEndingMatches(q.correctAnswer, q.sentence.trim().split(" ").pop())) {
        //   console.log(`Filtered out question due to adjective ending mismatch: ${q.sentence}`);
        //   return false;
        // }
        if (!isNaturalTranslation(q)) {
          console.log(`Filtered out question due to unnatural translation: ${q.translation}`);
          return false;
        }
        // All options should be plausible nouns and not all the same
        const plausible = plausibleOptions(q.options, q.correctAnswer);
        if (plausible.length < 2) {
          console.log(`Filtered out question due to insufficient plausible options: ${q.options.join(", ")}`);
          return false;
        }
        return true;
      });
        ...
        content: `You are a helpful assistant that translates simple English words or phrases into Latvian. Focus on the difficulty level: ${difficulty}. Respond in JSON format with 4 items like this format and two extra options to confuse the user: 
{
  ...
  "extraOption": ["...", "..."]
      ...
      {
        role: "user",
        content: `Provide 4 unique English to Latvian translations with two extra options.Focus on the difficulty level: ${difficulty}`,
      },
    ];
